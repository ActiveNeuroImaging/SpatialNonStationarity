...
file_name <- file.path(output_path,'SphereCoords_lowres.txt')  
write.table(mesh$loc, file = file_name, row.names = FALSE, col.names = FALSE)
file_name <- file.path(output_path,'SphereTris_lowres.txt')  
...
spde <- inla.spde2.matern(mesh, 
	...
	B.kappa = cbind(logkappa0, 0, -1, -1 * binary_variable), # Change for non-stationary
...
#spde <- 
 ...
 file_name <- file.path(output_path, paste0("NS_V3_Maps_", i, ".txt"))
...
spde <- inla.spde2.matern(mesh, 
	...
	B.kappa = cbind(logkappa0, 0, -1, -1 * (binary_variable)),
